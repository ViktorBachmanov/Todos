...
export const todosSlice = createSlice({
  ...
  reducers: {
    ...
    jumpToLastPage: (state) => {
      state.currentPage = Math.ceil(state.list.length / state.perPage) - 1;
      state.pageList = getPageTodos(
        state.list,
        state.perPage,
        state.currentPage
    ...
    save: (state, action: PayloadAction<Todo>) => {
      ...
      const arrayIndex = state.list.findIndex((todo) => {
        return todo.id === modifiedTodo.id;
      });
      state.list.splice(arrayIndex, 1, modifiedTodo);
      state.pageList = getPageTodos(
        state.list,
        state.perPage,
        state.currentPage
    ...
    add: (state, action: PayloadAction<string>) => {
      let maxTodoId: number = state.list.reduce((prev, current) => {
    ...
    remove: (state, action: PayloadAction<number>) => {
      ...
      state.list.splice(arrayIndex, 1);
      state.pageList = getPageTodos(
        state.list,
        state.perPage,
        state.currentPage
